"""多期限LLT因子（日频版）——基于广发多因子系列34的LLT低延迟趋势线选股策略。"""

import numpy as np
import pandas as pd

from factors.base import BaseFactor
from factors.registry import register_factor


@register_factor
class MultiHorizonLLTDaily(BaseFactor):
    """多期限LLT因子（日频版）：用低延迟趋势线替代MA均线，综合多期限信息选股。

    与MultiHorizonLLT的区别：
    - MultiHorizonLLT是周频因子，每周五输出一个信号
    - 本因子是日频因子，每个交易日输出一个信号
    - 用10日滚动回归替代25周滚动回归

    Parameters
    ----------
    periods : list[int], default None
        LLT期限列表，默认为 [3,5,10,20,30,60,90,120,180,240,270,300]。
    lookback_days : int, default 50
        回归系数计算的回溯天数，默认50天（约10周）。
    """

    name = "multi_horizon_llt_50d"
    description = "多期限LLT因子（日频版）"

    def __init__(
        self,
        periods: list[int] | None = None,
        lookback_days: int = 50,
    ) -> None:
        self.periods = periods or [3, 5, 10, 20, 30, 60, 90, 120, 180, 240, 270, 300]
        self.lookback_days = lookback_days

    @classmethod
    def _calc_llt(cls, prices: np.ndarray, period: int) -> np.ndarray:
        """计算LLT低延迟趋势线。

        LLT递推公式:
            LLT(1) = P(1)
            LLT(2) = P(2)
            LLT(t) = (2-2α)*LLT(t-1) - (1-α)²*LLT(t-2)
                     + (α-α²/4)*P(t) + (α²/2)*P(t-1) - (α-3α²/4)*P(t-2)

        Parameters
        ----------
        prices : np.ndarray
            价格序列。
        period : int
            均线周期，用于计算 α = 2/(N+1)。

        Returns
        -------
        np.ndarray
            LLT值序列。
        """
        n = len(prices)
        llt = np.full(n, np.nan)
        alpha = 2.0 / (period + 1)

        if n >= 1:
            llt[0] = prices[0]
        if n >= 2:
            llt[1] = prices[1]

        for t in range(2, n):
            if np.isnan(prices[t]) or np.isnan(prices[t - 1]) or np.isnan(prices[t - 2]):
                llt[t] = np.nan
            elif np.isnan(llt[t - 1]) or np.isnan(llt[t - 2]):
                llt[t] = prices[t]
            else:
                llt[t] = (
                    (2 - 2 * alpha) * llt[t - 1]
                    - (1 - alpha) ** 2 * llt[t - 2]
                    + (alpha - alpha ** 2 / 4) * prices[t]
                    + (alpha ** 2 / 2) * prices[t - 1]
                    - (alpha - 3 * alpha ** 2 / 4) * prices[t - 2]
                )
        return llt

    def generate_signals(
        self,
        data: pd.DataFrame,
        constituents: dict[str, set[str]] | None = None,
    ) -> pd.DataFrame:
        close = data["adj_close"].unstack("symbol").astype(float).sort_index()

        # 计算多期限LLT趋势线
        llt_dict = {}
        for period in self.periods:
            llt_values = close.apply(
                lambda col: pd.Series(
                    self._calc_llt(col.values, period), index=col.index
                )
            )
            llt_dict[period] = llt_values

        # 标准化因子: Ã = LLT_L / P
        a_dict = {}
        for period in self.periods:
            a_dict[period] = llt_dict[period] / close

        # 计算日收益率
        daily_ret = close.pct_change()

        # 初始化因子结果矩阵
        signals = pd.DataFrame(np.nan, index=close.index, columns=close.columns)

        # 滚动回归预测（日频）
        for i in range(self.lookback_days, len(close.index)):
            current_date = close.index[i]

            # 获取过去lookback_days天的收益率和因子值
            hist_dates = close.index[i - self.lookback_days:i]

            # 对每只股票计算回归系数
            for symbol in close.columns:
                # 获取该股票的历史收益率
                y = daily_ret.loc[hist_dates, symbol].values

                # 获取历史因子值 (各期限的Ã值)
                X_list = []
                for period in self.periods:
                    X_list.append(a_dict[period].loc[hist_dates, symbol].values)
                X = np.column_stack(X_list)

                # 去除NaN
                valid = np.isfinite(y) & np.all(np.isfinite(X), axis=1)
                if valid.sum() < self.lookback_days * 0.5:
                    continue

                y_valid = y[valid]
                X_valid = X[valid]

                # 回归: r = β0 + Σ βi * Ãi
                X_with_const = np.column_stack([np.ones(len(X_valid)), X_valid])
                try:
                    beta, _, _, _ = np.linalg.lstsq(X_with_const, y_valid, rcond=None)
                except np.linalg.LinAlgError:
                    continue

                # 用最新因子值计算预期收益率
                latest_a = np.array([
                    a_dict[period].at[current_date, symbol]
                    if pd.notna(a_dict[period].at[current_date, symbol])
                    else np.nan
                    for period in self.periods
                ])

                if np.any(np.isnan(latest_a)):
                    continue

                # E[r] = Σ βi * Ãi (不含常数项的贡献)
                expected_return = np.dot(beta[1:], latest_a)
                signals.at[current_date, symbol] = expected_return

        signals.index.name = "date"
        signals.columns.name = "symbol"
        return signals
