...
@register_factor
class GrossProfitabilityTrendQoq12m(BaseFactor):
    """最近连续 4 个季度近似点的 GP 截面标准分线性趋势。"""

    name = "gross_profitability_trend_qoq_12m"
    description = "海通39：GP 环比趋势，使用 4 个季度近似点计算斜率"

    def __init__(self, min_abs_denominator: float = 1e-12) -> None:
        self.min_abs_denominator = min_abs_denominator
        self.quarter_days = 21 * 3
    ...
    def generate_signals(
        ...
        lags = [self.quarter_days * 3, self.quarter_days * 2, self.quarter_days, 0]
